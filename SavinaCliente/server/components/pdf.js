...
const PREFIX_TITLE = 'Report of';
const KEYWORDS_DEFAULT = 'Savina Report request';

const HEADINGS = {
  DETAILS: 'Details',
  REPORT: 'Teleconsultation report',
...
SavinaReport.prototype.addAnswerAttachmentHeader = function addAnswerAttachmentHeader(data) {
  ...
  doc.font(DEFAULT_FONT_FAMILY).text(` - ${ data.user.specialtyId ? 'Specialist' : 'General practitioner' }`);
...
SavinaReport.prototype.writeDiagnosisTherapeuticPlan = function writeDiagnosisTherapeuticPlan() {
  return new Promise((resolve) => {
    ...
    if (diagnosis) {
      ...
      doc.text('unspecified care plan.').moveDown().moveDown();
    }
...
SavinaReport.prototype.writeDiagnosisExams = function writeDiagnosisExams() {
  return new Promise((resolve) => {
    ...
    if (diagnosisExams.length) {
      ...
      doc.text('unspecified tests.').moveDown();
    }
  ...
  return new Promise((resolve) => {
    ...
    if (diagnosis && diagnosis.checkupDate) {
      ...
      if (diagnosis.checkupExamsRequired) {
        list.push('Patient must come with performed tests.');
      } else {
        list.push('Patient can come without tests.');
      }

      if (diagnosis.checkupSameSpecialist) {
        list.push(`Checkup with Dr. ${ request.diagnosticAnswer.user.name }`);
      } else {
        list.push('Checkup with another medical professional.');
      }

      list.push(`Checkup type: ${ diagnosis.checkupMode.name }`);

      list.push(`Checkup date: ${ moment(diagnosis.checkupDate).format(FORMAT_DATE_HUMAN) }`);
      ...
      this.addDetail('Signature', request.diagnosticAnswer.user.name);
      this.addDetail('Date', moment(request.report.date).format(FORMAT_DATE_HUMAN));
    } else {
      doc.text('Unspecified checkup.').moveDown();
    }
  ...
  return new Promise((resolve) => {
    ...
    if (diagnosis) {
      if (diagnosis.confirmed) {
        doc.text('Confirmed hypothesis of diagnosis: ', PARAMS_CONTINUED);
      }
      ...
      doc.text('Unspecified confirmed diagnosis.').moveDown().moveDown();
    }
...
SavinaReport.prototype.writeAttachmentImages = function writeAttachmentImages(images) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown().font(HEADING_FONT_FAMILY).text(`Images (${ images.length })`)
...
SavinaReport.prototype.writeAttachmentDocuments = function writeAttachmentDocuments(documents) {
  return new Promise((resolve) => {
    ...
    doc.font(HEADING_FONT_FAMILY).text(`Documents (${ documents.length })`, DOC_MARGIN_LEFT, doc.y);
